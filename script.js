...
const state = {
  ...
  compare: readStore("cs:compare", [1, 3, 8]),
...
/*
  이동진 평론가 데이터는 공개적으로 확인 가능한 일부 영화만 수동 매핑합니다.
  공개 페이지의 짧은 한줄평과 별점만 저장하고, 사용자가 원문 맥락을 확인할 수 있도록 출처 링크를 함께 표시합니다.
*/
const leeCriticMap = {
  tt1375666: {
    rating: 4.0,
    summary: "플롯을 루빅스 큐브처럼 자유자재로 갖고 논다.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/xkPEmjWeL2K3W"
  },
  tt0468569: {
    rating: 5.0,
    summary: "내러티브와 스타일을 완벽히 장악한 자의 눈부신 활공.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/GO6MZrryOMNxa"
  },
  tt15398776: {
    rating: 4.5,
    summary: "구조와 플롯 자체가 강력한 핵폭탄이다.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/jxZQRVrdKBQbq"
  },
  tt0209144: {
    rating: 4.5,
    summary: "홀린 듯 사로잡히게 되는 구조의 마력.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/W9bEBPdvnxMpZ"
  },
  tt6751668: {
    rating: 4.5,
    summary: "상승과 하강으로 명징하게 직조해낸 신랄하면서 처연한 계급 우화.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/xzaQAkqBWN2g5"
  },
  tt5013056: {
    rating: 4.5,
    summary: "야심찬 뺄셈.",
    source: "익스트림무비 공개 게시물",
    url: "https://extmovie.com/movietalk/21643854"
  },
  "퓨리오사: 매드맥스 사가": {
    rating: 4.5,
    summary: "인물의 변화와 세계의 작동원리가 고스란히 담긴 기념비적 액션장면들의 연쇄.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/Vw8MLbxLqgEel"
  },
  "헤어질 결심": {
    rating: 5.0,
    summary: "파란색으로도 보이고 녹색으로도 보이는 그 옷처럼, 미결과 영원 사이에서 사무치도록.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/lP0E8mBn8Y2bx"
  },
  "에브리씽 에브리웨어 올 앳 원스": {
    rating: 5.0,
    summary: "그 모든 곳에서 그 모든 것이 한꺼번에 될 수 있다 해도 지금 이 순간을.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/y4ZMO6O8o72Op"
  },
  "괴물": {
    rating: 4.5,
    summary: "오해를 경유해서 이해에 이르는 경험 끝에 관객은 그 햇살 아래서 증인이 된다.",
    source: "왓챠피디아",
    url: "https://pedia.watcha.com/ko/comments/rdVMN6G54bMql"
  }
};

const leeCriticTitleIndex = Object.entries(leeCriticMap).reduce((index, [key, value]) => {
  if (!key.startsWith("tt")) index[normalizeKey(key)] = value;
  return index;
}, {});
const leeCriticAliases = {
  [normalizeKey("Inception")]: leeCriticMap.tt1375666,
  [normalizeKey("The Dark Knight")]: leeCriticMap.tt0468569,
  [normalizeKey("Oppenheimer")]: leeCriticMap.tt15398776,
  [normalizeKey("Memento")]: leeCriticMap.tt0209144,
  [normalizeKey("Parasite")]: leeCriticMap.tt6751668,
  [normalizeKey("Dunkirk")]: leeCriticMap.tt5013056,
  [normalizeKey("Furiosa: A Mad Max Saga")]: leeCriticMap[ "퓨리오사: 매드맥스 사가" ],
  [normalizeKey("Decision to Leave")]: leeCriticMap[ "헤어질 결심" ],
  [normalizeKey("Everything Everywhere All at Once")]: leeCriticMap[ "에브리씽 에브리웨어 올 앳 원스" ],
  [normalizeKey("Monster")]: leeCriticMap[ "괴물" ]
};
...
function saveState() {
  ...
  writeStore("cs:compare", state.compare);
...
function toggleCompare(id) {
  ...
  if (state.compare.includes(id)) {
    ...
    toast("영화 비교함에 추가했습니다.");
  }
  saveState();
  render(state.route);
}

function leeCriticFor(movie) {
  const candidates = [movie.imdbId, movie.title, movie.en, movie.originalTitle]
    .filter(Boolean)
    .map(value => String(value));
  for (const key of candidates) {
    const exact = leeCriticMap[key];
    if (exact) return exact;
    const normalized = normalizeKey(key);
    if (leeCriticTitleIndex[normalized]) return leeCriticTitleIndex[normalized];
    if (leeCriticAliases[normalized]) return leeCriticAliases[normalized];
  }
  return null;
}
...
function trailerPreview(movie) {
  const heroTrailerMap = {
    8: "uYPbbksJxIg"
  };
  const trailerKey = heroTrailerMap[movie.id];
  ...
  return `
    <iframe
      class="hero-trailer"
      src="https://www.youtube-nocookie.com/embed/${trailerKey}?${params.toString()}"
      title="${movie.title} 예고편 미리보기"
...
function movieCard(movie, options = {}) {
  ...
  const compared = state.compare.includes(movie.id);
  ...
  compareButton.className = `compare-toggle ${compared ? "selected" : ""}`;
  compareButton.type = "button";
  compareButton.dataset.action = "toggle-compare";
  compareButton.dataset.id = movie.id;
  compareButton.textContent = compared ? "비교✓" : "비교+";
  compareButton.setAttribute("aria-label", `${movie.title} 비교함 ${compared ? "삭제" : "추가"}`);
...
*/
function renderHome() {
  const heroIds = [8, 12, 2, 1, 3];
  const hero = movieById(heroIds[state.heroIndex % heroIds.length]);
  const popular = [2, 3, 10, 1, 12, 8, 11, 7].map(movieById);
  app.innerHTML = `
    <section class="home">
      <div class="hero real-hero" ${heroImage(hero)}>
        ${trailerPreview(hero)}
        <button class="hero-arrow left" type="button" data-action="hero-prev" aria-label="이전 추천 영화">‹</button>
        <div class="hero-content">
          <p class="eyebrow">${hero.id === 8 ? "제96회 아카데미 7관왕 수상" : "CineScore 추천 예고편"}</p>
          <h1>${hero.title}</h1>
        ...
        <button class="hero-arrow right" type="button" data-action="hero-next" aria-label="다음 추천 영화">›</button>
        <div class="hero-dots">
          ${heroIds.map((id, index) => `<button type="button" class="${index === state.heroIndex ? "active" : ""}" data-action="hero-set" data-index="${index}" aria-label="${movieById(id).title} 보기"></button>`).join("")}
        </div>
      </div>
    ...
    <section class="page layout-search">
      ...
      <section>
        ...
        <div class="load-more-panel">
          <button class="red-btn load-more" data-action="load-more-tmdb" ${(state.tmdbLoading || state.tmdbPage >= Math.min(state.tmdbTotalPages, 500)) ? "disabled" : ""}>
  ...
  app.innerHTML = `
    <section class="page">
      <div class="detail-hero">
        ...
        <div class="detail-copy">
          ...
          <div class="detail-actions">
            ...
            <button class="dark-btn" data-action="toggle-compare" data-id="${movie.id}">${state.compare.includes(movie.id) ? "비교함 삭제" : "비교함 추가"}</button>
      ...
      <div class="detail-sections">
        <section class="critic-panel">
          <h2>이동진 평론가</h2>
          ${lee ? `
            <div class="critic-card">
              <div class="critic-score"><span class="score">★</span><strong>${lee.rating.toFixed(1)}</strong><small>/5</small></div>
              <p>${lee.summary}</p>
              <a href="${lee.url}" target="_blank" rel="noreferrer">출처: ${lee.source}에서 확인하기 〉</a>
            </div>
          ` : `
            <div class="critic-card muted-card">
              <strong>등록된 이동진 평론가 데이터가 없습니다.</strong>
              <p>확인 가능한 출처가 있는 영화만 평점과 평론 요약을 표시합니다.</p>
            </div>
          `}
        </section>
        <section>
...
function compareCell(movie, key) {
  const lee = leeCriticFor(movie);
  const values = {
    poster: `${poster(movie)}<strong>${movie.title}</strong><button class="dark-btn" data-action="remove-compare" data-id="${movie.id}">비교에서 삭제</button>`,
    score: `${score(movie)}<span>관객 지수 ${movie.audience}%</span>`,
    lee: lee ? `<strong class="score">★ ${lee.rating.toFixed(1)}/5</strong><span>${lee.summary}</span><a href="${lee.url}" target="_blank" rel="noreferrer">${lee.source}</a>` : `<span class="muted">확인된 데이터 없음</span>`,
    genre: movie.genre.join(", "),
    runtime: `${movie.runtime}분`,
    year: movie.year,
    country: movie.country,
    director: movie.director,
    cast: movie.cast.slice(0, 4).join(", "),
    summary: movie.summary
  };
  return `<td>${values[key]}</td>`;
}

function renderCompare() {
  const list = state.compare.map(movieById).slice(0, 3);
  const rows = [
    ["poster", "영화"],
    ["score", "관객 평점"],
    ["lee", "이동진 평론가"],
  ...
  app.innerHTML = `
    <section class="page compare-page">
      <div class="section-head compare-head">
        <div>
          <h1>영화 비교</h1>
          <p>최대 3편까지 담아서 평점, 장르, 러닝타임, 평론가 평가를 한눈에 비교하세요.</p>
        </div>
        <div class="compare-actions">
          <button class="dark-btn" data-route="search">영화 추가하기</button>
          <button class="dark-btn" data-action="clear-compare">비교함 비우기</button>
        </div>
      </div>
      ${list.length ? `
        <table class="compare-table">
          <tbody>
            ${rows.map(([key, label]) => `<tr><th>${label}</th>${list.map(movie => compareCell(movie, key)).join("")}</tr>`).join("")}
          </tbody>
        </table>
      ` : empty("비교할 영화가 없습니다. 검색 결과나 상세 페이지에서 비교함에 추가해 주세요.")}
    </section>
  `;
}
...
function routeFromHash() {
  ...
  if (["home", "search", "detail", "recent", "mypage", "compare"].includes(hash)) return hash;
  return "home";
}
...
async function tmdbFetch(path, params = {}) {
  const query = new URLSearchParams({
    api_key: apiSettings.tmdbKey || TMDB_API_KEY,
    language: "ko-KR",
    ...params
  });
...
async function loadTmdbCatalog(reset = false) {
  if (state.tmdbLoading) return 0;
  if (reset) {
    state.tmdbPage = 0;
    state.tmdbTotalPages = 500;
    state.tmdbQuery = "";
  }
  if (state.tmdbPage >= Math.min(state.tmdbTotalPages, 500)) return 0;
  ...
  try {
    ...
    const added = Math.max(0, movies.length - before);
    if (!reset) toast(`TMDB 영화 ${added}개를 더 불러왔습니다.`);
    return added;
  } catch {
...
async function searchTmdbCatalog(query, reset = false) {
  ...
  try {
    ...
    (data.results || [])
      ...
      .forEach((item, index) => mergeMovie(mapTmdbMovie(item, index)));
    const added = Math.max(0, movies.length - before);
    if (!reset) toast(`TMDB 검색 결과 ${added}개를 더 불러왔습니다.`);
    return added;
  } catch {
...
async function hydrateFromTmdb(movie) {
  ...
  movie.tmdbId = tmdbId;
  movie.imdbId = detail.imdb_id || movie.imdbId;
  movie.originalTitle = detail.original_title || movie.originalTitle || movie.en;
...
document.addEventListener("click", async event => {
  ...
  if (action === "hero-prev") {
    const total = 5;
    state.heroIndex = (state.heroIndex - 1 + total) % total;
    render("home");
  }
  if (action === "hero-next") {
    const total = 5;
    state.heroIndex = (state.heroIndex + 1) % total;
    render("home");
  }
  if (action === "hero-set") {
    state.heroIndex = Number(target.dataset.index) || 0;
    render("home");
  }
  ...
  if (action === "remove-compare") {
    state.compare = state.compare.filter(id => id !== Number(target.dataset.id));
    saveState();
    toast("비교함에서 삭제했습니다.");
    render(state.route);
  }
  ...
  if (action === "load-more-tmdb") {
    if (state.query.trim()) {
      await searchTmdbCatalog(state.query);
    } else {
